...
def submit_quiz_attempt(quiz_id: str, student_id: str, answers: dict[str, str], consent_given: bool) -> str:
    ...
    with get_connection() as connection:
        connection.execute(
            """
            INSERT INTO quiz_attempts (id, quiz_id, student_id, score, percentage, status, started_at, submitted_at, consent_given)
            VALUES (?, ?, ?, 0, 0, 'submitted', ?, ?, ?)
            """,
            (attempt_id, quiz_id, student_id, started_at, started_at, int(consent_given)),
        )
...
def quiz_access_state(quiz: dict, now: datetime | None = None) -> tuple[bool, str | None]:
    now = now or datetime.now()
    start = parse_schedule(quiz.get("scheduled_start"))
    end = parse_schedule(quiz.get("scheduled_end"))
    if start and now < start:
